...
class OpenCVCamera(object, metaclass=dgpy_Module):
    ...
    def __init__(self, module_name, recdb, cameranum=0, capdevice = cv2.CAP_ANY, enablesettings=False):
        ...
        rec.rec.metadata.AddMetaDatum(snde.metadatum_str('nde_array-axis1_coord', 'X Position'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_str('nde_array-axis2_coord', 'Channel'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_str('nde_array-ampl_coord', 'Intensity'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_str('nde_array-ampl_units', 'Arb'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis0_inival', -1, 'pixels'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis1_inival', -1, 'pixels'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis2_inival', 0, 'unitless'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis0_step', 1, 'pixels'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis1_step', 1, 'pixels'))
        rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis2_step', 1, 'unitless'))
        ...
        try:
            while not self._quit:
                ...
                if ret:
                    ...
                    rec.rec.metadata.AddMetaDatum(snde.metadatum_str('nde_array-ampl_coord', 'Intensity'))
                    rec.rec.metadata.AddMetaDatum(snde.metadatum_str('nde_array-ampl_units', 'Arb'))
                    rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis0_inival', frame.shape[0]/(-2), 'pixels'))
                    rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis1_inival', frame.shape[1]/(-2), 'pixels'))
                    rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis2_inival', 0, 'unitless'))
                    rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis0_step', 1, 'pixels'))
                    rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis1_step', 1, 'pixels'))
                    rec.rec.metadata.AddMetaDatum(snde.metadatum_dblunits('nde_array-axis2_step', 1, 'unitless'))
